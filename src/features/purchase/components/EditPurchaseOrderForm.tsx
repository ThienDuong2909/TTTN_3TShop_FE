...
export default function EditPurchaseOrderForm({
  ...
  const [itemColorsSizes, setItemColorsSizes] = useState<{
    [key: number]: { colors: Color[]; sizes: Size[] };
  }>({});
  // Validate form
  ...
  const isProductDetailSelected = (
    ...
    return poForm.items.some(
      (item, index) => index !== currentIndex && item.MaCTSP === productDetailId
    );
  };
  ...
  const loadProductsBySupplier = async (supplierId: string) => {
    ...
    try {
      ...
      if (Array.isArray(response)) {
        ...
        productData =
          responseObj.products || responseObj.items || responseObj.result || [];
      }
  ...
  const getAvailableSuppliers = () => {
    const selectedProductIds = poForm.items
      .map((item) => item.MaSP)
      .filter((id) => id);
    ...
    try {
      ...
      for (const supplier of suppliers) {
        try {
          ...
          const hasProduct = supplierProducts.some(
            (p) => (p.MaSP || p.id)?.toString() === productId.toString()
          );
          if (hasProduct) {
  ...
  const loadProductDetails = async (index: number, productId: string) => {
    ...
    try {
      ...
      console.error(
        `Error loading product details for product ${productId}:`,
        error
      );
  ...
  const handleSubmit = () => {
    ...
    newErrors.expectedDeliveryDate = validateField(
      "expectedDeliveryDate",
      poForm.expectedDeliveryDate
    );

    // Validate items
    poForm.items.forEach((item, index) => {
      newErrors[`MaSP_${index}`] = validateItemField(index, "MaSP", item.MaSP);
      newErrors[`MaCTSP_${index}`] = validateItemField(
        index,
        "MaCTSP",
        item.MaCTSP
      );
      newErrors[`quantity_${index}`] = validateItemField(
        index,
        "quantity",
        item.quantity
      );
      newErrors[`unitPrice_${index}`] = validateItemField(
        index,
        "unitPrice",
        item.unitPrice
      );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
        ...
        <div>
          ...
          <Input
            ...
            onBlur={() =>
              setTouched((t) => ({ ...t, expectedDeliveryDate: true }))
            }
          />
          {isFieldInvalid(
            "expectedDeliveryDate",
                        ...
                        >
                          ...
                          >
                            <SelectValue placeholder="Chọn sản phẩm">
                              {filteredProducts.find((p) => p.id == item.MaSP)
                                ?.name ||
                                products.find((p) => p.id == item.MaSP)?.name ||
                                item.productName ||
                                "Chọn sản phẩm"}
                            </SelectValue>
                          </SelectTrigger>
                          ...
                          <SelectContent>
                            {(() => {
                              const productDetails =
                                itemProductDetails[index] || [];
                              const availableDetails = productDetails.filter(
                                (detail) =>
                                    ...
                                    <div className="flex items-center gap-2">
                                      {detail.Mau.MaHex && (
                                        <div
                                          ...
                                          style={{
                                            backgroundColor: detail.Mau.MaHex,
                                          }}
                                        />
                                      )}
                                      {detail.Mau.TenMau} -{" "}
