...
export default function Profile() {
  ...
  const getGenderText = (genderNumber: number) => {
    switch (genderNumber) {
      case 0:
        return "Nam";
      case 1:
  ...
  const getGenderNumber = (genderText: string) => {
    switch (genderText) {
      case "Nam":
        return 0;
      case "Nữ":
  ...
  const loadProfile = async () => {
    try {
      ...
      if (response.success) {
        ...
        setTimeout(
          () => setProgressiveStates((prev) => ({ ...prev, avatar: true })),
          300
        );
        setTimeout(
          () => setProgressiveStates((prev) => ({ ...prev, formFields: true })),
          500
        );
        setTimeout(
          () => setProgressiveStates((prev) => ({ ...prev, gender: true })),
          700
        );
        setTimeout(
          () => setProgressiveStates((prev) => ({ ...prev, actions: true })),
          900
        );
      }
    } catch (error) {
  ...
  const uploadImageToCloudinary = async (file: string | Blob) => {
    try {
  ...
  const handleAvatarUpload = async (event: { target: { files: any[] } }) => {
