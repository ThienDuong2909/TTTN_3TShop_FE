...
export default function OrderManagement() {
    ...
    .filter((order) => {
      ...
      return (
        ...
        (order.CT_DonDatHangs || []).some((ct) =>
          normalizeString(ct.ChiTietSanPham?.SanPham?.TenSP || "").includes(
            keyword
          )
        )
      );
  ...
  const hasReviews = (order) => {
    return order.CT_DonDatHangs.some(
  ...
  const hasReturnSlip = (order) => {
    return (
      order.HoaDon?.PhieuTraHang !== null &&
      order.HoaDon?.PhieuTraHang !== undefined
    );
  };
  ...
  // Handle view reviews
  const handleViewReviews = (order: any) => {
    const reviews = [];
    ...
    const returnData = {
      ...
      HoaDon: {
        ...
        DonDatHang: {
          ...
          SDTNguoiNhan: order.SDT,
          DiaChiGiao: order.DiaChiGiao,
          CT_DonDatHangs: order.CT_DonDatHangs.filter(
            (ct) => ct.SoLuongTra > 0
          ).map((ct) => ({
            DonGia: ct.DonGia,
  ...
  return (
    <div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-800 mb-2">
              {search.trim()
                ...
                : `Chưa có đơn hàng ${activeTab?.label.toLowerCase()}`}
            </h3>
      ...
      <ReturnDetailsModal
        ...
        onOpenChange={(open) =>
          setReturnDetailsModal({ isOpen: open, returnData: null })
        }
