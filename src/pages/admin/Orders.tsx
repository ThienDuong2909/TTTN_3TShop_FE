...
import { Order, DeliveryStaff } from "../../types/admin-order-type";
...
export default function Orders() {
  ...
  const calculateDeliveryStaffStats = useCallback(
    (allAssignedOrders: Order[]) => {
      const stats = {
        total: 0,
        placed: 0, // Đã đặt (status 1)
        approved: 0, // Đã duyệt (status 2)
  ...
  // Fetch all assigned orders for delivery staff to calculate statistics
  const fetchAllAssignedOrdersForStats = useCallback(async () => {
    if (!isDeliveryStaff) return;

    try {
      // Fetch all assigned orders without status filter to get all statuses
      const result = await getAssignedOrders({
        page: 1,
        limit: 1000, // Large limit to get all orders
        // Don't pass status to get all orders
      });

      if (result && result.success && result.data) {
        const allAssignedOrders = result.data.orders || result.data || [];
        calculateDeliveryStaffStats(allAssignedOrders);
      }
    } catch (error) {
      console.error("Error fetching all assigned orders for stats:", error);
    }
  }, [isDeliveryStaff, calculateDeliveryStaffStats]);

  // Fetch statistics for all order statuses
  const fetchOrderStatistics = useCallback(async () => {
    try {
      // If user is delivery staff, calculate statistics from assigned orders data
      if (isDeliveryStaff) {
        await fetchAllAssignedOrdersForStats();
        return;
      }
  ...
  }, [isDeliveryStaff, fetchAllAssignedOrdersForStats]);
      ...
      if (isDeliveryStaff) {
        ...
        for (const orderId of orderIds) {
          try {
            ...
            console.error(
              `Error confirming delivery for order ${orderId}:`,
              error
            );
            failedCount++;
          }
        ...
        if (successCount > 0) {
          toast.success(
            `Đã xác nhận giao hàng thành công ${successCount} đơn hàng`
          );
        }
        ...
        if (result && result.success) {
          toast.success(
            `Đã hoàn tất thành công ${result.data.success} đơn hàng`
          );
  ...
  const statisticsCards = [
    {
      title: isDeliveryStaff ? "Tổng đơn được phân công" : "Tổng đơn hàng",
      value: stats.total,
      icon: Package,
      color: "text-blue-600",
    },
    ...
    {
      title: isDeliveryStaff ? "Đang giao hàng" : "Đang giao",
      value: stats.shipping,
      icon: Package,
      color: "text-purple-600",
    },
    {
      title: isDeliveryStaff ? "Đã giao xong" : "Hoàn tất",
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <AdminHeader
        title={isDeliveryStaff ? "Đơn hàng được phân công" : "Quản lý đơn hàng"}
      />
