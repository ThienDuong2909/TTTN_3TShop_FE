...
export default function RevenueReport() {
  ...
  const getPeriodColumnName = () => {
    ...
    return lastGeneratedParams.reportType === "quarter"
      ? "Quý/Năm"
      : "Tháng/Năm";
  };
  ...
  const exportToExcel = async () => {
    ...
    worksheet.mergeCells(
      `${String.fromCharCode(
        65 + startCol - 1
      )}${startRow}:${String.fromCharCode(65 + titleEndCol - 1)}${startRow}`
    );
    worksheet.getCell(
      `${String.fromCharCode(65 + startCol - 1)}${startRow}`
    ).value = "3TSHOP";
    worksheet.getCell(
      `${String.fromCharCode(65 + startCol - 1)}${startRow}`
    ).font = {
    ...
    worksheet.mergeCells(
      `${String.fromCharCode(
        65 + startCol - 1
      )}${reportTitleRow}:${String.fromCharCode(
        65 + titleEndCol - 1
      )}${reportTitleRow}`
    );
    worksheet.getCell(
      `${String.fromCharCode(65 + startCol - 1)}${reportTitleRow}`
    ).value = getReportTitle();
    worksheet.getCell(
      `${String.fromCharCode(65 + startCol - 1)}${reportTitleRow}`
    ).font = {
    ...
    worksheet.mergeCells(
      `${String.fromCharCode(
        65 + startCol - 1
      )}${timeRangeRow}:${String.fromCharCode(
        65 + titleEndCol - 1
      )}${timeRangeRow}`
    );
    worksheet.getCell(
      `${String.fromCharCode(65 + startCol - 1)}${timeRangeRow}`
    ).value = `Thời gian: từ ${getDateRangeText()}`;
    worksheet.getCell(
      `${String.fromCharCode(65 + startCol - 1)}${timeRangeRow}`
    ).font = {
    ...
    infoData.forEach((info, index) => {
      ...
      worksheet.getCell(
        `${String.fromCharCode(65 + startCol - 1)}${rowNum}`
      ).value = info[0];
      worksheet.getCell(
        `${String.fromCharCode(65 + startCol - 1)}${rowNum}`
      ).font = {
    ...
    worksheet.mergeCells(
      `${String.fromCharCode(
        65 + startCol - 1
      )}${headerRowNum}:${String.fromCharCode(
        65 + startCol + 1
      )}${headerRowNum}`
    ); // B:D cho Tháng/Năm
    worksheet.mergeCells(
    ...
    processedData.forEach((item) => {
      ...
      worksheet.mergeCells(
        `${String.fromCharCode(
          65 + startCol - 1
        )}${currentRowNum}:${String.fromCharCode(
          65 + startCol + 1
        )}${currentRowNum}`
      ); // B:D
      worksheet.mergeCells(
    ...
    worksheet.getCell(
      `${String.fromCharCode(65 + startCol + 2)}${signatureRowNum}`
    ).value = "Người phê duyệt"; // Column E

    // Merge cells cho signature title (chiếm 3 cột E:G thay vì D:F)
    worksheet.mergeCells(
      `${String.fromCharCode(
        65 + startCol + 2
      )}${signatureRowNum}:${String.fromCharCode(
        65 + startCol + 4
      )}${signatureRowNum}`
    ); // E:G cho "Người phê duyệt"
    ...
    worksheet.getCell(
      `${String.fromCharCode(65 + startCol + 2)}${noteRowNum}`
    ).value = "(Ký và ghi rõ họ & tên)"; // Column E

    // Merge cells cho signature note (chiếm 3 cột E:G thay vì D:F)
    worksheet.mergeCells(
      `${String.fromCharCode(
        65 + startCol + 2
      )}${noteRowNum}:${String.fromCharCode(65 + startCol + 4)}${noteRowNum}`
    ); // E:G cho note
  ...
  `;
    ...
    a.href = url;
    a.download = `BaoCaoDoanhThu_${getDateRangeText().replace(
      /[\/\s]/g,
      "_"
    )}.doc`;
    a.click();
              ...
              <div className="flex items-end gap-4 flex-wrap">
                ...
                <div className="w-40">
                  ...
                  <Input
                    ...
                    value={startDate}
                    onChange={(e) =>
                      handleParamChange(undefined, e.target.value, undefined)
                    }
                  />
                </div>
                <div className="w-40">
                  ...
                  <div className="flex">
                    <div className="ml-3">
                      <p className="text-sm text-yellow-800">
                        ⚠️ Bạn đã thay đổi thông tin báo cáo. Vui lòng nhấn "Lấy
                        dữ liệu báo cáo" để cập nhật dữ liệu mới.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {processedData.length > 0 && isDataUpToDate && (
                  <div className="flex gap-2">
                    ...
                    <Select
                      value={exportType}
                      onValueChange={(value) =>
                        setExportType(value as "excel" | "word")
                      }
                    >
              ...
              {processedData.length > 0 && isDataUpToDate ? (
                  ...
                  <div className="max-h-80 overflow-y-auto border rounded-md">
                    <Table>
                      <TableHeader className="sticky top-0 bg-background">
                        <TableRow>
                          <TableHead className="w-[50%] text-center border-r">
                            {getPeriodColumnName()}
                          </TableHead>
                      ...
                      <TableBody>
                        {processedData.map((item, index) => (
                          <TableRow
                            key={`${item.period}-${item.nam}`}
                            className={
                              item.doanhThu === 0 ? "text-gray-400" : ""
                            }
                          >
                ...
                <div className="flex items-center justify-center h-80">
                  <div className="text-center text-muted-foreground">
                    ...
                    <p className="text-lg font-medium mb-2 text-orange-600">
                ...
                <div className="flex items-center justify-center h-80">
                  <div className="text-center text-muted-foreground">
                    ...
                    <p className="text-sm">
                      Vui lòng chọn thời gian và nhấn "Lấy dữ liệu báo cáo" để
                      xem dữ liệu
                    </p>
                  </div>
                </div>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-gray-50">
                        <TableHead className="font-bold text-center border text-gray-900">
                          {getPeriodColumnName()}
                        </TableHead>
                        <TableHead className="font-bold text-center border text-gray-900">
                          Doanh Thu (VNĐ)
                    ...
                    <TableBody>
                      {processedData.map((item, index) => (
                        <TableRow
                          key={`preview-${item.period}-${item.nam}`}
                          className={item.doanhThu === 0 ? "text-gray-400" : ""}
                        >
