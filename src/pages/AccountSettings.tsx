...
import { getAccountInfo, changePassword } from '../services/api';
import { AccountInfo, VaiTro } from '../services/api';
...
export default function AccountSettings() {
  ...
  try {
    ...
    if (response.success) {
      ...
      setTimeout(() => setProgressiveStates(prev => ({ ...prev, accountInfo: true })), 300);
      setTimeout(() => setProgressiveStates(prev => ({ ...prev, security: true })), 500);
    }
  } catch (error) {
...
const togglePasswordVisibility = (field: keyof typeof showPasswords) => {
  ...
  // Get role display text
    ...
    return roleMap[vaiTroObj.TenVaiTro] || vaiTroObj.TenVaiTro || 'Không xác định';
    };
        ...
        <CardContent className="space-y-6">
          ...
          <div 
            className={`grid grid-cols-1 md:grid-cols-2 gap-6 transition-all duration-500 ${
              progressiveStates.accountInfo ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
            }`}
          >
            {/* Email */}
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <div className="space-y-4 pt-4">
                    ...
                    <div className="relative">
                        ...
                        onChange={(e) => handlePasswordChange('xacNhanMatKhau', e.target.value)}
                    ...
                    <p className="text-sm text-yellow-800 font-medium mb-1">Yêu cầu mật khẩu:</p>
          ...
          <div 
            className={`bg-yellow-50 border border-yellow-200 rounded-lg p-4 transition-all duration-500 ${
              progressiveStates.security ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
            }`}
          >
            <div className="flex items-start gap-3">
              <Shield className="h-5 w-5 text-yellow-600 mt-0.5 flex-shrink-0" />
              <div>
                <h4 className="text-sm font-medium text-yellow-800 mb-1">Bảo mật tài khoản</h4>
                <p className="text-sm text-yellow-700">
                  Để bảo vệ tài khoản của bạn, hãy thường xuyên thay đổi mật khẩu và không chia sẻ thông tin đăng nhập với người khác.
                </p>
              </div>
            </div>
          </div>
