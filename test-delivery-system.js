...
async function testGetAssignedOrders() {
  ...
  try {
    const response = await fetch('http://localhost:8080/api/orders/delivery/assigned?page=1&limit=10', {
      method: 'GET',
      headers: {
...
async function testConfirmDelivery(orderId = 123) {
  ...
  try {
    const response = await fetch(`http://localhost:8080/api/orders/delivery/${orderId}/confirm`, {
      method: 'PUT',
      headers: {
        'Authorization': 'Bearer YOUR_JWT_TOKEN_HERE',
        'Content-Type': 'application/json'
...
function testPermissions() {
  ...
  mockUsers.forEach(user => {
    ...
    console.log(`   - Xem tất cả đơn hàng: ${user.canViewAllOrders ? '✅' : '❌'}`);
    console.log(`   - Xem đơn được phân công: ${user.canViewAssignedOrders ? '✅' : '❌'}`);
    console.log(`   - Xác nhận giao hàng: ${user.canConfirmDelivery ? '✅' : '❌'}`);
